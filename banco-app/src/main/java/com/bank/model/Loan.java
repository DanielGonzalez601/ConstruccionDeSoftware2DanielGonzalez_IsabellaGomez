...
public class Loan {
    ...
    public static Loan request(String clientId, String loanType, Money requestedAmount,
        ...
        if (clientId == null || clientId.isBlank()) throw new DomainException("Se requiere ID del cliente.");
        if (loanType == null || loanType.isBlank()) throw new DomainException("Se requiere tipo de préstamo.");
        if (requestedAmount == null || requestedAmount.isZero()) throw new DomainException("El monto solicitado debe ser mayor que cero.");
        if (termMonths <= 0) throw new DomainException("El plazo debe ser mayor que cero meses.");
    ...
    public void approve(Money approvedAmount, BigDecimal interestRate, int termMonths, int analystUserId) {
        ...
        if (approvedAmount == null || approvedAmount.isZero())
            throw new DomainException("El monto aprobado debe ser mayor que cero.");
        if (interestRate == null || interestRate.compareTo(BigDecimal.ZERO) <= 0)
            throw new DomainException("La tasa de interés debe ser mayor que cero.");
    ...
    public void reject(String reason, int analystUserId) {
        requireStatus(LoanStatus.UNDER_REVIEW, "rechazar");
        this.analystUserId = analystUserId;
        this.status = LoanStatus.REJECTED;
        domainEvents.add(new LoanRejected(loanId, clientId,
                reason != null ? reason : "No especificado", analystUserId));
    }
    ...
    public void markAsDisbursed(int analystUserId) {
        ...
        if (disbursementAccountNumber == null || disbursementAccountNumber.isBlank())
            throw new DomainException("La cuenta de desembolso no está definida.");
    ...
    private void requireStatus(LoanStatus required, String operation) {
        if (this.status != required)
            throw new DomainException("No se puede " + operation + " un préstamo en estado " + status + ". Requerido: " + required);
    }
