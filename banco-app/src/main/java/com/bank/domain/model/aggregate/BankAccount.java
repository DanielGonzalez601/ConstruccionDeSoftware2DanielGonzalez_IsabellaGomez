...
public class BankAccount {
    ...
    public static BankAccount open(String accountNumber, AccountType accountType,
        ...
        if (accountNumber == null || accountNumber.isBlank())
            throw new DomainValidationException("Se requiere número de cuenta.");
        if (ownerId == null || ownerId.isBlank())
            throw new DomainValidationException("Se requiere identificación del propietario.");
        if (currency == null || currency.isBlank())
            throw new DomainValidationException("Se requiere moneda.");
        BankAccount account = new BankAccount();
    ...
    public void deposit(Money amount) {
        ...
        if (amount.isZeroOrNegative())
            throw new DomainValidationException("Se requiere monto positivo para depósito.");
    ...
    public void withdraw(Money amount) {
        assertOperational();
        if (amount.isZeroOrNegative())
            throw new DomainValidationException("Se requiere monto positivo para retiro.");
        if (this.balance.isLessThan(amount))
            throw new InsufficientFundsException("Fondos insuficientes. Saldo: " + this.balance + ", solicitado: " + amount);
    ...
    public void block() {
        if (this.status == AccountStatus.CANCELLED)
            throw new AccountOperationNotAllowedException("No se puede bloquear una cuenta cancelada.");
        this.status = AccountStatus.BLOCKED;
    }

    public void unblock() {
        if (this.status == AccountStatus.CANCELLED)
            throw new AccountOperationNotAllowedException("No se puede desbloquear una cuenta cancelada.");
    ...
    private void assertOperational() {
        if (this.status != AccountStatus.ACTIVE)
            throw new AccountOperationNotAllowedException(
                    "Cuenta " + accountNumber + " is " + status + ". Operaciones no permitidas.");
    }

    private void assertNotCancelled() {
        if (this.status == AccountStatus.CANCELLED)
            throw new AccountOperationNotAllowedException("La cuenta está cancelada.");
    }
