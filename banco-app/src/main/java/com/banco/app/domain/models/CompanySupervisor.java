package com.banco.app.domain.models;

import com.banco.app.domain.enums.UserRoles;

import lombok.Getter;
import lombok.Setter;

import java.time.LocalDate;

@Getter
@Setter
public class CompanySupervisor extends User {

    private String companyId;

    public CompanySupervisor(String userId, String relationId, String fullName, String identificationNumber,
                             String email, String phone, LocalDate birthDate,
                             String address, String username, String passwordHash,
                             String companyId) {

        super(userId, relationId, fullName, identificationNumber, email, phone,
              birthDate, address, UserRoles.COMPANY_SUPERVISOR, username, passwordHash);

        this.companyId = companyId; //ID EMPRESA QUE ES SUPERVISOR.
    }

    @Override
    public String toString() {
        return "CompanySupervisor{userId='" + getUserId() +
               "', fullName='" + getFullName() +
               "', companyId='" + companyId + "'}";
    }
}